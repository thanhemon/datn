...
namespace Data.BLL
{
    public class FilmBLL : BusinessLogicLayer
    {
        ...
        SELECT 
            *
        FROM [Film]
        LEFT JOIN [CastOfFilm] ON [Film].[ID] = [CastOfFilm].[FilmId]
        LEFT JOIN [Cast] ON [CastOfFilm].[CastId] = [Cast].[ID]
        Left join [DirectorOfFilm] On [Film].[Id] = [DirectorOfFilm].[FilmId]
left join [Director] On [DirectorOfFilm].[DirectorId] = [Director].[Id]
        LEFT JOIN [CategoryDistribution] ON [Film].[ID] = [CategoryDistribution].[FilmId]
        LEFT JOIN [Category] ON [CategoryDistribution].[CategoryId] = [Category].[ID]
        WHERE [Film].[name] LIKE @name 
        OR [Cast].[name] LIKE @name
        OR [Category].[name] LIKE @name
Or [Director].[name] Like @name";
            }
            else
            {
                commandText = @"
...
";
            ...
            return await db.Execute_ToListAsync<FilmInfo>(
                commandText, CommandType.Text, new SqlParameter("@name", $"%{name}%"));
        }
     ...
     SELECT TOP ({0})
               [ID], [name], [description], [countryId], [productionCompany], [languageId],
               [releaseDate], [upvote], [downvote], [views], [duration], [thumbnail], [source],
               [createAt], [updateAt]
        FROM [MovieDB].[dbo].[Film]
        ORDER BY [upvote] DESC,[downvote] ASC ", count);


            return (await db.Execute_ToListAsync<Film>(commandText, CommandType.Text))
                .Select(f => ToFilmInfo(f)).ToList();
        }
        public async Task<List<FilmInfo>> GetNewFilmAsync(int count = 12)
        {
            string commandText = string.Format(@"
                SELECT TOP {0} 
                    [Film].[ID], 
        ...
        public async Task<List<FilmInfo>> GetFilmsByTagAsync(int tagId)
        {
            if (tagId <= 0)
            ...
            string commandText;
            if (includeTimestamp)
                commandText = string.Format(@"Select top {0} [Film].* from [Film], [TagDistribution]
                     where [Film].[ID] = [TagDistribution].[filmId]
                         and [TagDistribution].[tagId] = @tagId", 10); // Giả định bạn muốn lấy top 10
            else
                commandText = string.Format(@"Select *
                     from [Film], [TagDistribution]
                     where [Film].[ID] = [TagDistribution].[filmId]
                         and [TagDistribution].[tagId] = @tagId", 10); // Giả định bạn muốn lấy top 10

            return await db.Execute_ToListAsync<FilmInfo>(commandText, CommandType.Text, new SqlParameter("@tagId", tagId));
        }
        ...
        public bool Upvote(string filmId, string userId)
        {
            ...
            string checkExistingUpvoteCommand = @"SELECT COUNT(*) FROM [UserReaction]
                                          WHERE [userId] = @userId AND [filmId] = @filmId AND [upvoted] = 1";

            // Execute the query
            object result = db.ExecuteScalar(checkExistingUpvoteCommand, CommandType.Text,
                                             new SqlParameter("@userId", userId),
                                             new SqlParameter("@filmId", filmId));

            // Ensure result is not null and is an integer
            if (result == null || !(result is int))
                throw new Exception("Unexpected result from ExecuteScalar");

            int existingUpvoteCount = (int)result;

            if (existingUpvoteCount > 0)
            {
                // User has already upvoted this film, no need to insert again
                return false;
            }

            // Check if the user has already reached the upvote limit
            string checkLimitCommand = @"SELECT COUNT(*) FROM [UserReaction]
                                 WHERE [userId] = @userId AND [upvoted] = 1";

            int userUpvoteCount = (int)db.ExecuteScalar(checkLimitCommand, CommandType.Text,
                                                        new SqlParameter("@userId", userId));

            if (userUpvoteCount >= 20)
            {
                // Return failure without proceeding to upvote
                return false;
            }

            // Proceed with the upvote operation
            string insertUpvoteCommand = @"INSERT INTO [UserReaction] ([userId], [filmId], [upvoted])
                                   VALUES (@userId, @filmId, 1)";

            db.ExecuteNonQuery(insertUpvoteCommand, CommandType.Text,
                               new SqlParameter("@userId", userId),
                               new SqlParameter("@filmId", filmId));

            string updateFilmCommand = @"UPDATE [Film] SET [upvote] = (
            ...
            int affected = db.ExecuteNonQuery(updateFilmCommand, CommandType.Text,
                                              new SqlParameter("@filmId", filmId));
        ...
        public async Task<List<FilmInfo>> GetVotedFilmsByUserIdAsync(string userId)
        {
            if (string.IsNullOrEmpty(userId))
                throw new Exception("@'userId' must not be null or empty");
            ...
            if (includeTimestamp)
            {
                commandText = @"Select [Film].* 
                        from [Film]
                        inner join [UserReaction] on [Film].[ID] = [UserReaction].[filmId]
                        where [UserReaction].[userId] = @userId
                        and [UserReaction].[upvoted] = 1";
            }
            else
            {
                commandText = @"Select *
            ...
            return await db.Execute_ToListAsync<FilmInfo>(commandText, CommandType.Text, new SqlParameter("@userId", userId));
        }
        public async Task<UpdateState> DeleteVotedFilmAsync(string userId, string filmId)
        {
            if (string.IsNullOrEmpty(userId))
                throw new Exception("@'userId' must not be null or empty");
            if (string.IsNullOrEmpty(filmId))
            ...
            string commandText = @"DELETE FROM [UserReaction]
                           WHERE [userId] = @userId
                           AND [filmId] = @filmId";

            int affectedRows = await db.ExecuteNonQueryAsync(commandText, CommandType.Text,
                new SqlParameter("@userId", userId),
                new SqlParameter("@filmId", filmId));

            return affectedRows > 0 ? UpdateState.Success : UpdateState.Failed;
        }
